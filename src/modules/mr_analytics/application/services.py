...
class MRAnalyticsService:
    """Application service for MR analytics"""
    
    def calculate_metrics_from_mr(self, mr: MergeRequest) -> MRMetrics:
        """Calculate metrics from MergeRequest domain model"""
        return MRMetrics(
            mr_iid=mr.iid,
            title=mr.title,
            author=mr.author,
    ...
    def _calculate_review_rounds(self, mr: MergeRequest) -> int:
        """Calculate number of review rounds"""
        if not mr.comments:
            return 0
    ...
    def _calculate_comment_density(self, mr: MergeRequest) -> float:
        """Calculate comment density (comments per line changed)"""
    ...
    def _calculate_formal_approval(self, mr: MergeRequest) -> int:
        """Calculate formal approval (1 if approval without comments, 0 otherwise)"""
    ...
    def _calculate_response_time(self, mr: MergeRequest) -> float | None:
        """Calculate response time to first comment in hours"""
    ...
    def get_top_authors_by_lines(self, mrs: List[MergeRequest], limit: int = 10) -> List[Dict[str, Any]]:
        """Get top authors by lines changed"""
        author_stats = {}
    ...
    def get_mr_complexity_score(self, mr: MergeRequest) -> float:
        """Calculate MR complexity score"""
        lines_score = min((mr.additions + mr.deletions) / 100, 1.0)
        comments_score = min(len(mr.comments) / 10, 1.0)
        approvals_score = min(len(mr.approvals) / 5, 1.0)
        
        return (lines_score * 0.5 + comments_score * 0.3 + approvals_score * 0.2)
